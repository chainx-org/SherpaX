...
# parachain
nohup ./sherpax -d ./data/sherpax --keystore-path ./keystore --collator --discover-local --parachain-id 2013 --rpc-cors=all --ws-port 9977 --rpc-port 8545 --port 40041 --chain ./config/basic.json -lruntime=debug,aura=trace,cumulus-collator=trace,cumulus-network=trace,cumulus-consensus=trace --  --chain ./config/rococo-local-raw.json --discover-local --port 40042 > data/log.2013 2>&1 &
