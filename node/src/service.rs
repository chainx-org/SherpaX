...
pub struct BasicRuntimeExecutor;

impl sc_executor::NativeExecutionDispatch for BasicRuntimeExecutor {
    type ExtendHostFunctions = ();

    fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
        basic_runtime::api::dispatch(method, data)
    }
...
pub fn basic_build_import_queue(
    client: Arc<
        TFullClient<
            Block,
            basic_runtime::RuntimeApi,
            NativeElseWasmExecutor<BasicRuntimeExecutor>,
        >,
    >,
    config: &Configuration,
...
/// This is the actual implementation that is abstract over the executor and the runtime api.
#[sc_tracing::logging::prefix_logs_with("Parachain")]
async fn start_sherpax_node_impl<RuntimeApi, Executor, RB, BIQ, BIC>(
    parachain_config: Configuration,
    polkadot_config: Configuration,
    id: ParaId,
    _rpc_ext_builder: RB,
...
pub async fn start_sherpax_parachain_node(
    ...
    start_sherpax_node_impl::<sherpax_runtime::RuntimeApi, SherpaxRuntimeExecutor, _, _, _>(
        parachain_config,
        polkadot_config,
        id,
        |_| Ok(Default::default()),
